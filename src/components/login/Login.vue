<template>
  <div
    style="display: flex;justify-content: center;height: 90%;margin-top: 2.5%;margin-bottom: 2.5%;"
    class="pic"
  >
    <div
      style="margin-top: 12%;
...
"
    >
      ...
      <div style="display: flex;justify-content: center;">
        <div class="deng1">
          <input
            type="text"
            v-model="code1"
            style="width: 50%;height: 100%;outline: none;border: none;"
            placeholder="请输入验证码"
          />
      ...
      <div
        @click="gologin"
        style="color:#fff;align-items: center; display: flex;justify-content: center;background:  #0C9CEC;
       width:320px;height:40px;border-radius: 10px;margin: 0 auto;margin-top:55px;"
      >登录</div>
    </div>
  </div>
</template>
    ...
    gologin: function() {
      ...
      if (main.phone != main.phone1) {
        ...
        api.login(data).then(response => {
          console.log(response);
          console.log(response.data.data.companyId);
          ...
          if (response.data.data == "手机号为null") {
            this.$message({
              message: "手机号未填写"
            });
            return;
          }
          if (response.data.data == "验证码为null") {
            this.$message({
              message: "验证码未填写"
            });
            return;
          }
          if (response.data.data == "验证码不正确") {
            this.$message({
              message: "验证码不正确"
            });
            return;
          }
          if (response.data.data.companyId != "undefined") {
            window.sessionStorage.setItem(
              "companyId",
              response.data.data.companyId
            );
            console.log(window.sessionStorage.getItem("companyId"));
            this.$message({
              message: "登录成功",
              type: "success"
            });
