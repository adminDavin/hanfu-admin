      ...
      <el-tab-pane label="活动管理" name="first" style="color:orange;">
        <div>
          <el-button
            type="primary"
            @click="addactive"
            style="margin-right: 60px; margin-bottom: 20px; overflow: hidden;
           margin-top: 20px;margin-left: 20px;float:right;"
          >添加活动</el-button>
        </div>
          ...
          >
            ...
            <el-form-item prop="activiyType" label="活动策略" label-width="120px">
              <el-select v-model="value" placeholder="请选择" @change="changedata()">
                <el-option
                  v-for="(item,index) in strategyData"
                  :key="index"
        ...
        <el-table :data="activeData" style="margin-top: 30px;" height="500px">
          ...
          <el-table-column label="活动名称" width="100" align="center">
            <template slot-scope="scope">
              ...
              <p
                style="width: 80px;margin: 0 auto; text-overflow:ellipsis;overflow: hidden;white-space: nowrap;"
              >{{ scope.row.activityName }}</p>
          ...
          <el-table-column prop="strategyId" label="活动策略" width="200" align="center">
            <template slot-scope="scope">
              <div
                @click="strategy(scope.row.strategyId,scope.row.id,scope.row.type)"
                style="color: #fff;font-size: 14px;
              width: 98px;height:40px;  border-radius: 4px;margin: 0 auto;color: #00D1B2;text-overflow:ellipsis;overflow: hidden;line-height:40px ;text-align: center;white-space: nowrap;cursor:pointer;"
              >查看</div>
          ...
          <el-table-column label="操作" align="center">
            <template slot-scope="scope">
              <el-button
                type="primary"
                size="mini"
                @click="editActive(scope.row.id,scope.row.strategyId,scope.row.type)"
              >活动详情</el-button>
              <el-button
                type="primary"
              ...
              <el-button
                ...
                style="background:#E04C2F;border:1px solid  #E04C2F;"
                size="mini"
                @click="control(scope.row.id)"
              >开启活动</el-button>
              <el-button
                type="primary"
                v-if="scope.row.isTimingStart==1"
                style="background:#bbb;border:1px solid  #bbb;"
                size="mini"
                @click="control(scope.row.id)"
              >关闭活动</el-button>
              <el-button
                ...
                plain
                icon="el-icon-delete"
                @click="delecteActive(scope.row.id)"
                size="mini"
              >删除</el-button>
            </template>
          </el-table-column>
